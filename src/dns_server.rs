...
impl DnsServer {
  pub fn new(settings: DnsSettings) -> DnsServer {
    Self {
      settings,
    }
  }
  ...
  fn lookup(&self, query_name:  &str, query_type: DnsQueryType) -> Result<DnsPacket, Error> {
    let server = (match self.settings.servers.choose(&mut rand::thread_rng()) {
      Some(x) => x.as_str(),
      None => "8.8.8.8"
    }, 53);
  ...
  fn lookup_internal(&self, query_name: &str, query_type: DnsQueryType, server: (&str, u16)) -> Result<DnsPacket, Error> {
    let socket = UdpSocket::bind(("0.0.0.0", self.settings.backup_port))?;
    println!("okay here");
  ...
  fn local_lookup(&self, query_name: &String, _query_type: DnsQueryType) -> Option<Vec<DnsRecord>> {
    // TODO what should _query_type do in this function?
    match self.settings.records.get(query_name) {
      Some(x) => Some(x.clone()),
      None => None
    }
  }
